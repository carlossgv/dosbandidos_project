...
def get_orders_clover(merchant_id, initial_date, finish_date):
    ...
    response = requests.get(
        ...
        f'&filter=createdTime<{finish_date}&expand=payments&expand=serviceCharge&limit=500', headers=headers)
...
def get_payment_details(merchant_id, payment_id):
    headers = {'accept': 'application/json', 'authorization': f'Bearer {os.environ.get("DOSBANDIDOS_CLOVER_API")}'}
    response = requests.get(
        f'https://api.clover.com/v3/merchants/{merchant_id}/payments/{payment_id}?expand=taxRates', headers=headers)
...
def get_refund_details(merchant_id, refund_id):
    headers = {'accept': 'application/json', 'authorization': f'Bearer {os.environ.get("DOSBANDIDOS_CLOVER_API")}'}
    response = requests.get(
        f'https://api.clover.com/v3/merchants/{merchant_id}/refunds/{refund_id}?expand=serviceCharge', headers=headers)
...
def get_refunds_list(merchant_id, initial_date, finish_date):
    ...
    response = requests.get(
        ...
        f'&filter=createdTime<{finish_date}&limit=500', headers=headers)
